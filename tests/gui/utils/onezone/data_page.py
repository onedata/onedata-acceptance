...
class Space(Element):
    ...
    harvesters_discovery = NamedButton(
        ".one-list-level-2 .item-header", text="Harvesters, Discovery"
    )
...
class HarvestersPage(PageObject):
    ...
    invite_harvester_using_token = NamedButton(
        ".generate-invite-token-action.btn", text="Invite harvester using token"
    )
...
class DataPage(GenericPage):
    ...
    def get_visible_space_headers_list(self, id_param="name") -> List[SpaceHeader]:
        return [
            header for header in self.spaces_header_list if getattr(header, id_param)
        ]

    def get_visible_spaces_list(self, id_param="name") -> List[Space]:
