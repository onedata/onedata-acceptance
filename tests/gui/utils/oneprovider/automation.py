...
class Task(Element):
    ...
    def click_on_drag_handle(self) -> Any:
        self.web_elem.find_element(By.CSS_SELECTOR, ".task-drag-handle").click()

    def click_on_option_in_task(self, option: Any) -> Any:
        if option == "Audit log":
            ...
            self.web_elem.find_element(
                By.CSS_SELECTOR, ".view-task-pods-activity-action-trigger"
            ).click()
        elif option == "Time series":
            self.web_elem.find_element(
                By.CSS_SELECTOR, ".view-task-time-series-action-trigger"
            ).click()
...
class ParallelBox(Element):
    ...
    def scroll_to_bottom_of_task_in_parallel_box(self, task_id: Any) -> Any:
...
class WorkflowLane(Element):
    ...
    def scroll_to_first_task_in_parallel_box(self, number: Any) -> Any:
...
class WorkflowVisualiser(PageObject):
    ...
    workflow_lanes = WebItemsSequence(
        ".visualiser-elements .workflow-visualiser-lane", cls=WorkflowLane
    )
...
class WorkflowExecutionPage(PageObject):
    ...
    available_workflow_list = WebItemsSequence(
        ".atm-workflow-schemas-list .list-entry", cls=Workflow
    )
    initial_value_store = WebItemsSequence(
        ".inputStores-collapse .inputStore-field", cls=InitialValueStore
    )
    ...
    def click_on_background_in_workflow_visualiser(self) -> Any:
