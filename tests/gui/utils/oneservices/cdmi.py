...
def get_item_type(item_path):
    return ('container' if item_path.split('/')[-1].startswith('dir')
            else 'object')
...
class CDMIClient(object):
    ...
    def create_file(self, path, text=''):
        ...
        data = {'value': text}
        return http_put(self.ip, self.port, parsed_path, headers=headers,
    ...
    def read_from_file(self, path, read_range=None):
        ...
        headers.update(self.auth_header)
        return http_get(self.ip, self.port, parsed_path, headers=headers,
                        default_headers=False).content

    def read_metadata(self, path, metadata=""):
        item_type = get_item_type(path)
        parsed_path = parse_path(path, item_type, add_cdmi_prefix=True)
        parsed_path = ('{path}?metadata:{metadata}'
                       .format(path=parsed_path, metadata=metadata))
        headers = {'X-CDMI-Specification-Version': self.cdmi_version}
        headers.update(self.auth_header)
        return http_get(self.ip, self.port, parsed_path, headers=headers,
