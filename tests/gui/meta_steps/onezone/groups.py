...
@then(parsers.re('(?P<user>\w+) invites (?P<user2>\w+) to group '
                 '"(?P<group_name>.*)" using Oneprovider web GUI'))
def create_group_token_using_op_gui(selenium, user, user2, oz_page, group_name,
                                    tmp_memory, displays, clipboard):
    item_type = 'token'

    click_generate_token_in_subgroups_list(selenium, user, group_name,
                                           oz_page)
...
def add_subgroups_using_op_gui(selenium, user, oz_page, parent, group_list,
    ...
    for child in parse_seq(group_list):
        create_group_token_using_op_gui(selenium, user, user, oz_page,
                                        parent, tmp_memory,
                                        displays, clipboard)
