...
@wt(
    parsers.parse(
        'user of {browser_id} creates "{lambda_name}" lambda from '
...
@wt(
    parsers.parse(
        "user of {browser_id} uploads all lambda dumps from "
...
def assert_downloaded_and_uploaded_lambda_dumps_the_same(
    browser_id: Any, lambda_name: Any, tmpdir: Any
) -> Any:
