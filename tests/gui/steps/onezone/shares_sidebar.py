...
@wt(
    parsers.parse(
        'user of {browser_id} sees space name "{space_name}" of '
...
def assert_share_name_in_shares_sidebar(
    selenium, browser_id, oz_page, share_name
):
    shares_list = oz_page(selenium[browser_id])["shares"].shares_sidebar_list
    share_names_list = {share.name for share in shares_list}

    assert (
        share_name in share_names_list
    ), f"Share {share_name} not in shares sidebar"
