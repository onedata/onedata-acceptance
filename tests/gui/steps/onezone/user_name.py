...
@wt(parsers.parse('user of {browser_id} activates edit box by clicking on '
                  'the user name in Profile page'))
@repeat_failed(timeout=WAIT_BACKEND)
def activate_user_name_edit_box_in_oz(selenium, browser_id, oz_page):
    oz_page(selenium[browser_id])['profile'].rename_user_name()
...
def click_on_btn_for_user_name_edit_box_in_oz(selenium, browser_id,
                                                   btn, oz_page):
    getattr(oz_page(selenium[browser_id])['profile'].edit_user_name_box, btn).click()


@wt(parsers.parse('user of {browser_id} sees that the user name displayed '
                  'in Profile page is "{expected_user_name}"'))
@repeat_failed(timeout=WAIT_FRONTEND)
def assert_correct_user_name_in_oz(selenium, browser_id,
                                       expected_user_name, oz_page):
