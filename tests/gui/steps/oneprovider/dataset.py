...
def assert_toggle_checked_on_item_in_ancestor_list(
    browser_id, selenium, modals, kind, name
):
    driver = selenium[browser_id]
...
@wt(
    parsers.parse(
        "user of {browser_id} cannot click {toggle_type} write "
...
@wt(
    parsers.parse(
        'user of {browser_id} sees path to root file: "{path}" '
...
@wt(
    parsers.parse(
        "user of {browser_id} sees that one of two listed datasets"
...
def click_protection_toggle_in_ancestor_list(
    browser_id, selenium, modals, name, modal_name, toggle_type
):
    driver = selenium[browser_id]
    toggle = getattr(
