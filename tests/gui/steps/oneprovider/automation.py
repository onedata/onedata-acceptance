...
@wt(parsers.parse('user of {browser_id} sees {status} status for pod in modal "Function pods activity"'))
@repeat_failed(timeout=WAIT_FRONTEND)
def assert_status_in_events_monitor(selenium, browser_id, op_container, modals,
                                     task_name):
    switch_to_iframe(selenium, browser_id)
    page = op_container(selenium[browser_id]).automation_page
    workflow_visualiser = page.workflows_page.workflow_visualiser
