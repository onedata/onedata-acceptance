...
@wt(
    parsers.parse(
        "user of {browser_id} sees that modification date of item "
...
@wt(
    parsers.parse(
        'user of {browser_id} selects first "{num_files_to_select}"'
...
@repeat_failed(timeout=WAIT_FRONTEND)
def deselect_all_items_from_file_browser(
...
@repeat_failed(timeout=WAIT_BACKEND)
def assert_empty_dir_msg_in_file_browser(
    browser_id: str, tmp_memory: TmpMemory
) -> None:
    browser = tmp_memory[browser_id]["file_browser"]
...
@wt(
    parsers.parse(
        "user of {browser_id} sees that contents of downloaded "
...
@wt(
    parsers.parse(
        "user of {browser_id} sees alert that file cannot be "
