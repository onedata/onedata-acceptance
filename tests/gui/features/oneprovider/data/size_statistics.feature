Feature: Size statistics tests
  ...
  Scenario: User sees size statistics after turning on size statistics
    ...
    And user of browser sees that "Size Statistics" item displayed in modal Directory Details is not active
    ...
    And user of browser clicks "Files" of "space1" space in the sidebar
    And user of browser waits for displayed size in data row of "dir1" to be is "3 B"
    And user of browser clicks on size statistics tag for "dir1" directory in file browser
