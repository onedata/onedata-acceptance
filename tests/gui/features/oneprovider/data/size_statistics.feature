Feature: Size statistics tests
  ...
  Scenario: User sees size statistics after clicking on size statistics icon
    When user of browser clicks "space1" on the spaces list in the sidebar
    And user of browser clicks "Files" of "space1" space in the sidebar
    And user of browser sees file browser in files tab in Oneprovider page
    And user of browser sees that displayed size in data row of "dir1" is "3 B"
  ...
  Scenario: User does not see size statistics after turning them off
    ...
    And user of browser unchecks size statistics toggle in providers settings on providers page
    And user of browser clicks on "Disable" button in modal "Disable directory statistics"

    And user of browser clicks "Files" of "space1" space in the sidebar
    Then user of browser waits for displayed size in data row of "dir1" to be "—"
