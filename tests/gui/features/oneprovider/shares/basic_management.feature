Feature: Basic share management in Oneprovider GUI
  ...
  Scenario: User sees shared status tag for directory after sharing it
    ...
    And user of browser sees that "Directory details" modal is opened on "Shares" tab
  ...
  Scenario: User shares a directory and opens its view in full Onezone interface from modal
    ...
    And user of browser sees share's file browser in files tab in Oneprovider page
    Then user of browser sees that item named "dir1" has appeared in share's file browser on single share view
  ...
  Scenario: User sees new files in single share view in full Onezone interface after adding them to shared directory
    ...
    And user of browser clicks and presses enter on item named "dir1" in share's file browser
    Then user of browser sees that item named "20B-0.txt" has appeared in share's file browser on single share view
  ...
  Scenario: User does not see files in single share view in full Onezone interface after removing them from shared directory
    ...
    And user of browser clicks and presses enter on item named "dir2" in share's file browser
    And user of browser sees only items named ["dir3", "file1", "file2"] in share's file browser
    ...
    And user of browser clicks and presses enter on item named "dir2" in share's file browser
    Then user of browser sees only items named ["dir3", "file2"] in share's file browser
  ...
  Scenario: User can change working directory using breadcrumbs
    ...
    And user of browser clicks and presses enter on item named "dir3" in share's file browser
    Then user of browser sees that current working directory path visible in share's file browser is as follows: /dir2/dir3
    And user of browser clicks and presses enter on item named "dir4" in share's file browser
  ...
  Scenario: User downloads files from shared directory on single share view in full Onezone interface
    ...
    Then user of browser downloads item "file1" in share's file browser by clicking and pressing enter and then sees that content of downloaded file is equal to: "11111"
  ...
  Scenario: Removing directory which contained shared directory removes only share's files but not share itself
    ...
    And user of browser sees share's file browser on single share view
    And user of browser sees "SHARED FILES HAVE BEEN DELETED" instead of share's file browser
