...
#Chyba te teściki do multiusera trzeba przerzucić
 Scenario: User fails to rename group because of lack in privileges
   When user of browser opens group "group1" members subpage
    ...
    And user of browser clicks on "Rename" button in group "group1" menu in the sidebar
    And user of browser writes "group2" into rename group text field
    And user of browser confirms group rename using confirmation button
    Then user of browser sees that error modal with text "insufficient privileges" appeared
