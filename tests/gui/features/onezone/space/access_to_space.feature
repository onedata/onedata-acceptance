Feature: Test user has access to space via group membership
  ...
  Scenario: User has access to space via group membership
    ...
    And user of browser1 adds group "group1" to space using copied token and sees following message: ".*joined.*"
  ...
  Scenario: User fails to join to the space because the space was deleted
    ...
    Then user of browser1 fails to join space using received token and sees error message on modal: "is invalid"
    And user of browser1 closes error modal with info about invalid target with id of "space1" space
