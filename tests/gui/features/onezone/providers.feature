...
#    Then user of browser sees "oneprovider-1" is on the providers list
  ...
  Scenario: User sees that if space is unsupported in provider in Data, that provider is not displayed in providers
    When user of browser clicks on Data in the main menu
    And user of browser clicks "space1" on the spaces list in the sidebar
    And user of browser clicks Providers of "space1" in the sidebar
    # ?!?!?! user clicks on menu button on "oneprovider-1" on providers list
    And user of browser revokes "space1" space support in "oneprovider-1" provider in Onepanel
    And user of browser clicks on Providers in the main menu
    Then user of browser sees that "oneprovider-1" provider is not in providers list in data sidebar
