Feature: Management of invite tokens in Onezone GUI, with admin user
  ...
  Scenario: User successfully consumes harvester to space invite token until usage limit is not expired
    ...
    Then user of browser2 succeeds to consume token for "harvester1" harvester and sees following message: ".*joined.*"
    ...
    Then user of browser2 succeeds to consume token for "harvester2" harvester and sees following message: ".*joined.*"
    ...
    Then user of browser2 fails to consume token for "harvester3" harvester and sees following message: "Consuming token failed"
  ...
  Scenario: User successfully cleans up obsolete tokens
    ...
    And user of browser1 succeeds to consume token for "group1" group and sees following message: ".*joined.*"
