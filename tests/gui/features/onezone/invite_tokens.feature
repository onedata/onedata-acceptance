...
#          unit: GiB
  ...
  Scenario: User sees right Invite tokens after filtering them
    Given user admin has no harvesters
    And using REST, user admin creates "harvester8" harvester in "onezone" Onezone service
    And using REST, user admin creates "harvester9" harvester in "onezone" Onezone service
...
#    User must be an admin to be able to create cluster invite tokens, oneprovider2??
    ...
    Then user of browser1 sees exactly 9 item(s) on tokens list in tokens sidebar # expected 7 plus 2 admin tokens ??
    ...
    And user of browser1 sees that there is token named "space_token_2" on tokens list
    And user of browser1 chooses "space1" name Invite filter in tokens sidebar
...
#    And user of browser1 chooses "admin" name Invite filter in tokens sidebar
#    And user of browser1 sees exactly 3 item(s) on tokens list in tokens sidebar
    ...
    And user of browser1 sees exactly 2 item(s) on tokens list in tokens sidebar
    And user of browser1 sees that there is token named "group_token_1" on tokens list
    And user of browser1 sees that there is token named "group_token_2" on tokens list
    ...
    And user of browser1 sees exactly 2 item(s) on tokens list in tokens sidebar
    And user of browser1 sees that there is token named "harvester_token_1" on tokens list
    And user of browser1 sees that there is token named "harvester_token_2" on tokens list
