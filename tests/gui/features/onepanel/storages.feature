Feature: Storage management using onepanel
  ...
  Scenario Outline: User uploads files on freshly supported space on newly created storage
    ...
    And user of browser1 uses spaces select to change data space to "<space>"
    And user of browser1 sees file browser in data tab in Oneprovider page
    And user of browser1 sees that current working directory displayed in breadcrumbs is <space>
    ...
    And user of browser1 sees that current working directory displayed in breadcrumbs is <breadcrumbs>
