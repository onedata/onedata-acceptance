Feature: Provider management in Onepanel GUI
  ...
  Background:
    ...
    And opened [browser1, space_owner_browser] with [admin, space-owner-user] signed in to [emergency interface of Onepanel, onezone] service
  ...
  Scenario: User deregisters provider, registers it again and sees that provider is working
    ...
    And user of space_owner_browser generates space support token for space "helloworld" and sends it to user of browser1
