...
basic operations in Onepanel using REST API in Oneclient tests.
"""
...
@wt(parsers.parse('using REST, {user} changes storage "{type}" named "{storage}" parameter '
                    '"{parameter}" to "{value}" at provider "{provider}"'))
def modify_storage_parameter(user, type, storage, parameter, value, provider,
    ...
    pprint.pprint(hosts)
    pprint.pprint(onepanel_credentials)
    pprint.pprint(storages)
    ...
    modify_storage_parameters(user, provider, storage_id, storage, modify_params,
                              onepanel_host, onepanel_credentials)
    pass
