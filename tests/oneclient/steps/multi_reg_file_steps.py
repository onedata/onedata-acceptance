...
@wt(parsers.re('(?P<user>\w+) writes "(?P<text>.*)" to previously opened '
...
@when(parsers.re('(?P<user_name>.*) writes "(?P<data>.*)" at offset (?P<offset>.*) to '
                 '(?P<file>.*) on (?P<client_node>.*)'))
...
@wt(parsers.re('(?P<user>\w+) changes (?P<file>.*) size to (?P<new_size>.*) bytes '
               'on (?P<client_node>.*)'))
def do_truncate(user, file, new_size, client_node, users):
    do_truncate_base(user, file, new_size, client_node, users)


@wt(parsers.re('(?P<user>\w+) fails to change (?P<file>.*) size to (?P<new_size>.*) '
