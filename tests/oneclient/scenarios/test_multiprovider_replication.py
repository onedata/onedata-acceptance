...
@scenario('Create empty file on one provider and read it on the '
          'other provider')
...
@scenario('Create nonempty file on one provider and override its contents on '
          'the other provider')
def test_remote_file_override(env_description_file):
    pass


@scenario('Create nonempty file on one provider and remove it on the other '
          'provider')
def test_remote_file_removal(env_description_file):
    pass


@scenario('Create nonempty file, append remotely, append locally and read '
          'both')
def test_sequential_appends(env_description_file):
    pass


@scenario('Concurrently write disjoint ranges and read the same on both '
          'providers')
