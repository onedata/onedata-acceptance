...
from tests.performance.types import ExceptionQueue, PerformanceParams, int_parameter
...
class TestTransferOnf(AbstractPerformanceTest):
    ...
    def test_transfer_onf(
        self, hosts: Hosts, users: Users, env_desc: EnvDesc, params: PerformanceParams
    ) -> list[Result]:
        client1 = mount_performance_client(
            users, hosts, env_desc, "user1", "oneclient-1", "client11"
        )
        client2 = mount_performance_client(
            users, hosts, env_desc, "user2", "oneclient-2", "client21"
        )

        files_number = int_parameter(params, "files_number")
        file_size = int_parameter(params, "files_size")
        ...
        test_result = _execute_test(
            client2, files_number, file_size / 1024, threads_num, dir_path_2
        )
...
def _execute_test(
    ...
    for interval in intervals:
        j = i + interval
        workers.append(Thread(target=_copy_files, args=(client, i, j, dir_path, queue)))
        i = j
