        ...
        try:
            ...
            if item_name.startswith('dir'):
                ...
                if isinstance(item_desc, int):
                    assert_msg = ('Directory {} in {} has wrong number of '
                                  'children. Expected: {}, got: {}'
                                  .format(item_name, cwd, item_desc,
                                          len(item_children)))
                    assert len(item_children) == item_desc, assert_msg

                else:
                    check_files_tree(item_desc, item_children,
                                     os.path.join(cwd, item_name), ls_fun,
                                     assert_file_content_fun)
            else:
                assert_file_content_fun(os.path.join(cwd, item_name),
                                        str(item_desc))
...
def assert_ace(priv, item_type, ace, name, num, path):
    ...
    set_priv.sort()
    assert ace['identifier'].startswith(name), ('Identifier in {} ACE is '
                                                'not {}'.format(num, name))
    assert ace['acetype'] == acetype, ('Type in {} ACE is not {}'
                                       .format(num, acetype))
