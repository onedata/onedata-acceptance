...
from tests.mixed.utils.default_workflow_executions import DefaultExecution
...
def upload_all_workflows_from_automation_examples_rest(
    ...
    ALL_WORKFLOW_WITHOUT_INPUT_FILES = [
        f.split('.')[0] for f in os.listdir(upload_workflow_path(None)) if not
        os.path.isdir(upload_workflow_path(f))]
    ALL_WORKFLOW_WITH_INPUT_FILES = [
        f.split('.')[0] for f in os.listdir(upload_workflow_path(None)) if
        os.path.isdir(upload_workflow_path(f))]
    ...
    for workflow_name in ALL_WORKFLOW_WITH_INPUT_FILES:
        path = ''.join([upload_workflow_path(workflow_name), '/', workflow_name, '.json'])
        upload_workflow_rest(hosts, zone_name, users, user, inventory,
...
def get_store_id_of_workflow(store_name, workflow_name):
...
def execute_all_workflows(user, users, hosts, host, spaces, space, workflows,
    ...
    for workflow in workflows:
        ...
        if hasattr(default_execution, workflow.replace('-', '_')):
            content_ = getattr(default_execution, workflow.replace('-', '_'))()
            for content in content_:
                # map store name into store_id
                content = {get_store_id_of_workflow(key, path):
...
def execute_workflow_rest(
    ...
    ATM_WORKFLOW_EXECUTION_ID.append(
        {workflow_name: workflow_execution_api.schedule_workflow_execution(data)
         })
...
def wait_for_workflow_executions(user, users, host, hosts, space, spaces):
    executions = list_workflow_executions(user, users, host, hosts, space,
                                          spaces, phase='ongoing')
    if any(executions):
        raise RuntimeError(f'workflows with ids {executions} did not finished')


@wt(parsers.parse('using REST, {user} sees successful execution of all '
                  'workflows on space "{space}" in {host}'))
def assert_successful_workflow_executions(user, users, host, hosts, space,
                                          spaces):
    executions = list_workflow_executions(user, users, host, hosts, space,
                                          spaces, phase='suspended')
    if any(executions):
        raise Exception(f'workflows with ids {executions} were suspended')
