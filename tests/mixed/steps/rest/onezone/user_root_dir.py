...
@wt(parsers.parse('using REST, {user} fails to remove user root dir of '
                  'space "{space_name}" in {host}'))
def try_to_remove_user_root_dir(users, user, hosts, host, space_name,
                                 tmp_memory):
    try:
...
def try_to_create_file_in_user_root_dir(users, user, hosts, host, tmp_memory,
...
@wt(parsers.parse('using REST, {user} fails to move '
                  'user root dir of "{space_name}" in {host}'))
def try_to_move_user_root_dir(user, users, hosts, host,
                                              tmp_memory, space_name, cdmi):
    try:
        client = cdmi(hosts[host]['ip'], users[user].token)
        client.move_item_by_id(tmp_memory['space_root_dir'][space_name], '/')
