Feature: Directories times tests
  ...
  Scenario Outline: User changes directory using <client2> and using <client1> sees that modification time has changed
    ...
    # call sleep, to be sure that time of above and below operations is different
    And user1 is idle for 2 seconds
    And using <client1>, user1 copies modification time of item named "dir1" in "space1" space in oneprovider-1
    And using <client1>, user1 copies status-change time of item named "dir1/dir2" in "space1" space in oneprovider-1
