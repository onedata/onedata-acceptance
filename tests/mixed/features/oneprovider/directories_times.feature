Feature: Directories times tests
 ...
 Scenario Outline: User renames directory using <client2> and check status-change time using <client1>
    ...
    And using <client1>, user1 sees that status-change time of item named "dir2" in "space1" space is equal to modification time in <host1>
    And using <client1>, user1 sees that status-change time of item named "dir2" in "space1" space is equal to access time in <host1>
  ...
  Scenario Outline: User changes directory using <client2> and using <client1> he sees that modification time has changed
    ...
    # call sleep, to be sure that time of above and below operations is different
    And user1 waits 2 second
    And using <client2>, user1 succeeds to create directory named "dir1/dir2" in "space1" in <host2>
    Then using <client1>, user1 sees that modification time of item named "dir1" in "space1" space is greater than access time in <host1>
    And using <client1>, user1 sees that modification time of item named "dir1" in "space1" space is equal to status-change time in <host1>
  ...
  Scenario Outline: User changes directory using <client1> and using <client2> sees that modification time has changed
    ...
    And using <client2>, user1 succeeds to create directory named "dir1/dir2" in "space1" in <host2>
    Then using <client1>, user1 sees that modification time of item named "dir1" in "space1" space is not earlier than 70 seconds ago in <host1>
