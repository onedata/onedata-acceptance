Feature: Shares API tests
  ...
  Scenario: User reads file content using the command from "Download file content" from API section in share file details modal
    ...
    And user of browser clicks on menu for "file1" file in shares file browser
    And user of browser clicks "Information" option in data row menu in shares file browser
