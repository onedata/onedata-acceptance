...
def unit_tests_directory_in_client_container(hosts, client):
    ...
    cmd = [
        ...
        "cp",
        LOCAL_UNIT_TESTS_DIR,
        "{0}:{1}".format(hosts[client]["container-id"], CONTAINER_ONEDATA_FS_DIR),
    ]
...
def run_tests_in_container(client, cfg, hosts, users, python_version):
    ...
    dump_test_cfg_cmd = "echo '{}' > {}".format(
        yaml.dump(test_cfg), CONTAINER_TEST_CFG_FILE
    )
...
def fetch_test_results(client, python_version, hosts):
    results_path = os.path.join("test-reports", "python{}".format(python_version))
    ...
    cmd = [
        "docker",
        "cp",
        "{0}:{1}".format(hosts[client]["container-id"], CONTAINER_REPORTS_XML_DIR),
        results_path,
    ]
    check_call_with_logging(cmd)
