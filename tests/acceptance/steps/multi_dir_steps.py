...
@when(parsers.re('(?P<user>\w+) creates directories (?P<dirs>.*) on '
                 '(?P<client_node>.*)'))
...
@wt(parsers.re('(?P<user>\w+) fails to create directories (?P<dirs>.*) on '
               '(?P<client_node>.*)'))
...
@wt(parsers.re('(?P<user>\w+) deletes empty directories (?P<dirs>.*) on '
               '(?P<client_node>.*)'))
...
@wt(parsers.re('(?P<user>\w+) deletes non-empty directories (?P<dirs>.*) on '
               '(?P<client_node>.*)'))
