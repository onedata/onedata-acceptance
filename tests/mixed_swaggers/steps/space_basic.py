...
@when(parsers.re('using (?P<client>.*), (?P<user>.+?) sees provider '
                 'named "(?P<provider_name>.+?)" with hostname matches that of '
                 '"(?P<provider>.+?)" provider in "(?P<host>.+?)" Onezone '
                 'service'))
def assert_provider_has_given_name_and_known_hostname_in_oz(client, request,
                                                            user, provider_name,
                                                            provider, host,
                                                            users, hosts,
                                                            selenium, oz_page):
        ...
        if client == "rest":
            ...
            assert_provider_has_name_and_hostname_in_oz_gui(
                selenium, user, oz_page, provider,
