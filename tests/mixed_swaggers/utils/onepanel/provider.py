...
from tests.mixed_swaggers.onepanel_client import (OneproviderApi,
                                                  ProviderDetails,
...
def modify_provider_in_op_panel_using_rest(user, users, provider_host, hosts,
    ...
    provider_mod_rq = ProviderDetails(name=new_provider_name,
                                      subdomain_delegation=False,
                                      domain=new_domain)
    provider_api.modify_provider(provider_mod_rq)


def deregister_provider_in_op_panel_using_rest(user, users, provider_host,
                                               hosts):
    user_client = login_to_panel(user, users[user].password,
                                 hosts[provider_host]['hostname'])
...
def register_provider_in_op_using_rest(user, users, hosts, config):
    ...
    storages = {provider_api.get_storage_details(storage_id).name:
                provider_api.get_storage_details(storage_id)
        ...
        for field in ('type', 'insecure', 'readonly'):
            ...
            if expected_val:
                ...
                assert expected_val == actual_val, \
                    'Attribute {} for storage named {} does not match ' \
                    'expected value. Expected: {}, got: {}'.format(
                        field, storage_name, expected_val, actual_val)
