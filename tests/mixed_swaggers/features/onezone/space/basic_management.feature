Feature: Basic management
  ...
  Scenario Outline: User removes space using <client1> and using <client2> he sees that it has been removed
    When using <client1>, user1 removes space named "space1" in "z1" Onezone service
    Then using <client2>, user1 sees that space named "space1" has disappeared from "z1" Onezone service
