...
def get_images_option(
    ...
    add_image_to_images_cfg(
        rest_cli_image, "rest-cli", "--rest-cli-image", images_cfg, pull
    )
    ...
    if test_type in ["oneclient", "mixed", "performance"]:
        ...
        with open(env_file_path, "r") as f:
            if yaml.load(f, yaml.Loader).get("openfaas", False):
                add_image_to_images_cfg(
                    oc_image, "oneclient", "--oc-image", images_cfg, pull
                )
                add_image_to_images_cfg(
...
def add_image_to_images_cfg(
    image: str | None, service_name: str, option: str, images_cfg: list[str], pull: bool
) -> None:
    if not image:
...
def env_errors_exists(testsuite: ElementTree.Element | None) -> bool:
    ...
    for testcase in testcases:
        ...
        if skipped is not None:
            if re.match(
                ".*environment error.*", skipped.attrib["message"].lower(), re.I
            ):
                return True
    return False
...
def clean_env(
    ...
    container = docker.ps(
        all=True, quiet=True, filters=[("name", TEST_RUNNER_CONTAINER_NAME)]
    )
    if container:
...
def main() -> None:
    ...
    parser.add_argument(
        "--timeout", action="store", help="Onenv wait timeout", dest="timeout"
    )
...
SERVICE_TO_IMAGE: dict[str, str] = {
    ...
    "openfaas-lambda-result-streamer": (
        "docker.onedata.org/openfaas-lambda-result-streamer"
    ),
}
...
def get_current_branch() -> str:
    if "bamboo_planRepository_branchName" in os.environ:
        ...
        print(
            '[INFO] ENV variable "bamboo_planRepository_branchName" is set to '
    ...
    if branch_name == "HEAD":
        raise ValueError(
            "Could not resolve branch name - repository in detached HEAD state. "
